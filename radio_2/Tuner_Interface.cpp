...
#include <Wire.h> // library from https://github.com/stm32duino/Arduino_Core_STM32/tree/master/libraries/Wire
...
uint16_t Tuner_Init(void) //return 1 = sucsess
{
  ...
  uint16_t j;
  uint16_t limit;
  const unsigned char *p = reinterpret_cast<const unsigned char *>(&tuner_init_tab[0]);
  unsigned char t[20];
